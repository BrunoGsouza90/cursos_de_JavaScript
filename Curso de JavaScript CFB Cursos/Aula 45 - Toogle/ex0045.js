let caixas =[...window.document.querySelectorAll('.lista_cursos')]
let itemsSelecionados = [...window.document.querySelectorAll('.lista_cursos input[type="radio"]')]

function atualizarElementos(){
    caixas =[...window.document.querySelectorAll('.lista_cursos')]
    itemsSelecionados = [...window.document.querySelectorAll('.lista_cursos input[type="radio"]')]
}
...
criarNovoCursoAntes.addEventListener('click', () =>{
    ...
    novo_lista_cursos.style.background = 'gray'
    ...
    itemsSelecionados.forEach(item =>{
        if(item.checked){
            div_cursos.insertBefore(novo_lista_cursos, item.parentElement)
            encontrado = true
        }
    })
...
criarNovoCursoDepois.addEventListener('click', () =>{
    ...
    itemsSelecionados.forEach(item =>{
        if(item.checked){
            div_cursos.insertBefore(novo_lista_cursos, item.parentElement.nextElementSibling)
            encontrado = true
        }
    })
...
itemsSelecionados.forEach(item =>{
    item.addEventListener('click', ()=>{
        for(let item in itemsSelecionados){
            if(itemsSelecionados[item].checked){
                itemsSelecionados[item].parentElement.style.background = 'lightgreen'
                window.alert(`"${(itemsSelecionados[item].previousElementSibling.innerHTML)}" selecionado.`)
            }else{
                itemsSelecionados[item].parentElement.style.background = 'gray'
            }
        }
    })
})
